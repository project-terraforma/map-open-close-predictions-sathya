...
def analyze_location(location):
    ...
    yelp_not_closed = yelp_data.get('is_closed') == False
    ...
    if fsq_status == 'no_data' and yelp_reviews >= 20 and yelp_not_closed and has_corroboration:
        logit += 0.8
        open_score = 1.0 / (1.0 + math.exp(-logit))
        score_breakdown.append({"signal": "yelp_corroboration", "weight": 0.8, "description": "Yelp open + corroboration (boost)"})

    # Hard override: if Yelp explicitly says closed, trust it (98% accurate)
    if yelp_data.get('is_closed') == True:
        open_score = min(open_score, 0.15)
        score_breakdown.append({"signal": "yelp_closed", "weight": -3.0, "description": "Yelp: EXPLICITLY CLOSED (override)"})
        evidence_parts.append("Yelp: CLOSED")
